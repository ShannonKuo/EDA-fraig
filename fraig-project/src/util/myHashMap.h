...
{
public:
   //HashKey(size_t k1, size_t k2, bool i1, bool i2 ) 
   //       : key1(k1), key2(k2), inv1(i1), inv2(i2) {}
   HashKey(vector<size_t> k, size_t s, bool f, bool i1, bool i2)
          : key(k), sim(s), fec(f), inv1(i1), inv2(i2) {}
   size_t operator() () const { 
      if (fec == 0) return (key[0] + key[1]);
      else {
         int cnt = 0;
   ...
   bool operator == (const HashKey& k) const {
      if (fec == 0){ 
         if (key[0] == k.key[0] && key[1] == k.key[1]){ 
            if (inv1 == k.inv1 && inv2 == k.inv2) return true;}
         else if(key[1] == k.key[0] && key[0] == k.key[1]){
            if (inv2 == k.inv1 && inv1 == k.inv2) return true;}
         return false;
      }
      else{
         if (sim == k.sim || sim == ~k.sim) return true; 
         else return false;
      } 
   }
